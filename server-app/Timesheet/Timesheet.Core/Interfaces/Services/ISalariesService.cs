...
namespace Timesheet.Core.Interfaces.Services
{
    public interface ISalariesService
    {
        ...
        Task<MonthlySalaryModel> GetMonthlySalaryAsync(string userId, int year, int month, CancellationToken cancellationToken);
        Task<MonthlyTimeConsumingSummaryModel> GetMonthlyTimeConsumingSummaryAsync(string userId, int year, int month, CancellationToken cancellationToken);
    }
}
